...
import { ChoiceField, NumberField, TextField } from "@/components/forms/controls";
...
export default function OnboardingPage() {
  ...
  const finish = async () => {
    ...
    if (presetId) {
      await applyPreset.mutateAsync({ yearId, presetId, replaceExisting: false });
    }
    if (template !== "none") {
  ...
  return (
    <div className="mx-auto flex min-h-svh w-full max-w-lg flex-col px-4 pb-10 pt-safe">
          ...
          <>
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {t("Let us set up your school year. You can change all of this later.")}
              </p>
            </div>
              ...
              {presets.data?.map((preset) => (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="block text-xs text-muted-foreground">
                      {t("{count} subjects", { count: String(preset.subjectCount) })}
                    </span>
                  </span>
                  {presetId === preset.id ? (
                    <CheckIcon className="mt-0.5 size-4 text-primary" />

...
function resolveSettings<TData>(
  ...
  // State pairs live in the store. Keep the settings object limited to stable
  // configuration and callbacks without manufacturing unused bindings.
  const rest = { ...options }
  delete rest.events
  delete rest.defaultEvents
  delete rest.view
  delete rest.defaultView
  delete rest.date
  delete rest.defaultDate
  delete rest.dayCount
  delete rest.defaultDayCount
  delete rest.selection
  delete rest.defaultSelection
  delete rest.interactions
  delete rest.defaultInteractions
  delete rest.viewSettings
  delete rest.defaultViewSettings
  delete rest.loading
...
function useEventCalendarSelector<TData = unknown, TSelected = unknown>(
  ...
  const state = useSyncExternalStore(
    ...
    instance.getState,
    instance.getState
  ) as EventCalendarState<TData>
  const selected = selector(state)
  // The registered component is used read-only in Avermate, so correctness is
  // more important than mutating refs during render for a selector micro-cache.
  // Keep the comparison option in the public contract for registry parity.
  void options?.isEqual
  return selected
}

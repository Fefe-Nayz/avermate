...
function CascaderNav({ className, ...props }: CascaderNavProps) {
  const defaultProps = {
    ...
    className: cn(
      ...
      "flex shrink-0 flex-col gap-1 border-b border-border/60 px-1.5 py-1",
      className
    ),
  }
...
function CascaderBack({ className, children, ...props }: CascaderBackProps) {
  ...
  const defaultProps = {
    ...
    onClick: () => popLevel(),
    className: cn(
      "flex shrink-0 items-center justify-center rounded-md text-muted-foreground outline-hidden transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:ring-2 focus-visible:ring-ring/50",
...
function CascaderBreadcrumb({
  ...
  const defaultProps = {
    ...
    className: cn(
      "flex min-w-0 shrink-0 items-center gap-0.5 pt-1.5 pb-0.5 text-xs text-muted-foreground",
      ...
      >
        ...
        {segment.type === "ellipsis" ? (
          ...
          <button
            ...
            className="max-w-32 truncate rounded-sm outline-hidden transition-colors hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring/50"
          >
            {segment.node.label}
          </button>
        ) : (
  ...
  return (
    ...
    >
      ...
      <ComboboxPrimitive.Input
        ...
        className={cn(
          "h-full w-full min-w-0 flex-1 bg-transparent px-1.5 text-sm outline-hidden placeholder:text-muted-foreground disabled:cursor-not-allowed disabled:opacity-50",
          backVisible && "ps-0",
          className
        )}
        {...props}

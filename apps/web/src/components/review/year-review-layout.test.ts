...
function assertContained(rect: Rect, container: Rect) {
  ...
  assert.ok(
    ...
    `left edge ${rect.x} is outside ${container.x}`
  )
  assert.ok(
    rect.y >= container.y - epsilon,
    `top edge ${rect.y} is outside ${container.y}`
  )
  assert.ok(
...
describe("calculateYearReviewLayout", () => {
  it("keeps the canonical story and desktop controls in an asymmetric safe rect", () => {
    ...
    assert.ok(
      Math.abs(layout.storyRect.width / layout.storyRect.height - 390 / 693) <
        1e-12
    )
    assert.equal(
      layout.storyRect.x + layout.storyRect.width / 2,
      safeRect.width / 2
    )
    ...
    assertContained(
      toViewportRect(layout.previousButtonRect, safeRect),
      safeRect
    )
  ...
  it("hides side navigation and contains the story and close control on mobile", () => {
    ...
    assertContained(layout.storyRect, {
      x: 0,
      y: 0,
      width: safeRect.width,
      height: safeRect.height,
    })
    assertContained(layout.closeButtonRect, {
      x: 0,
      y: 0,
      width: safeRect.width,
      height: safeRect.height,

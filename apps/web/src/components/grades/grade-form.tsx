...
export function GradeForm({
  ...
  const { graph, subjects, year, yearId, period, scale } = useYear();
  ...
  const impact = useMemo(() => {
    ...
    const simulated = new SubjectGraph(
      graph.subjects.map((subject) => {
        ...
        const kept = initial?.id
          ? subject.grades.filter((grade) => grade.id !== initial.id)
          : subject.grades;
  ...
  const create = useMutation({ ...orpc.grades.create.mutationOptions(), ...onSaved });
  const update = useMutation({ ...orpc.grades.update.mutationOptions(), ...onSaved });
        ...
        <Field orientation="horizontal">
          <FieldLabel htmlFor="composite-toggle" className="flex-1">
            ...
            <span className="block text-xs font-normal text-muted-foreground">
              {t("Written and oral, or several exercises with their own weights")}
            </span>
          </FieldLabel>
          <Switch
            id="composite-toggle"
              ...
              >
                <div className="flex items-center gap-2 pb-2">
                  <input
                    ...
                    placeholder={t("Part {number}", { number: String(index + 1) })}

...
export function isUnlockablePalette(value: unknown): value is UnlockablePalette {
  return (
    typeof value === "string" &&
...
export function customThemeCss(
  ...
  const entries = Object.entries(overrides).filter(([token, value]) =>
    (THEME_TOKENS as readonly string[]).includes(token) && value.trim().length > 0,
  );

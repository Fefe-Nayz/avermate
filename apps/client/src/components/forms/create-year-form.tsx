...
export const CreateYearForm = ({
  ...
  const createYearSchema = z.object({
    ...
    defaultOutOf: z.coerce
      ...
      .max(1000, t("CREATE_YEAR_FORM_OUT_OF_MAX_ERROR")),
  });
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: (data) => {
      console.log("CreateYearForm onSuccess called", {
        data,
      ...
      if (onYearCreated) {
        console.log("Calling onYearCreated with:", data.id);
        onYearCreated(data.id);
      } else if (!isUpdating) {
        console.log(
          "No onYearCreated callback, navigating to:",
          `/onboarding/${data.id}`
        );
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      defaultOutOf: initialData?.defaultOutOf
        ? initialData.defaultOutOf / 100
        : 20,
    },
  });
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="mx-1">
                  <FormLabel>
                    {t("CREATE_YEAR_FORM_NAME_FIELD_LABEL")}
                  </FormLabel>
                  <FormControl>
                    <Input
                      type="text"
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="mx-1">
                  ...
                  <FormDescription>
                    {t("CREATE_YEAR_FORM_OUT_OF_FIELD_DESCRIPTION")}
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
